...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, CustomThunk, DisplayName="Parse Node into Struct", Category="YAML", meta=(CustomStructureParam="Struct"))
	...
	DECLARE_FUNCTION(execParseIntoStruct_BP) {
		...
		if (!StructProperty)
			UE_LOG(LogYamlParsing, Error, TEXT("ParseIntoStruct did not receive an Struct Property: '%s'"), *Stack.MostRecentProperty->GetName())
