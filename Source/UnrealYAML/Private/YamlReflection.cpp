...
FString FYamlParseResult::ScopeName() const {
    return FString::Join(ScopesStack, TEXT("."));
}

void FYamlParseResult::PushStack(const FString& Property) {
    ScopesStack.Add(Property);
}

void FYamlParseResult::PushStack(const FYamlNode& Key) {
    ScopesStack.Add(Key.Scalar());
}

void FYamlParseResult::PushStack(const int32 Index) {
    ScopesStack.Add(FString::Printf(TEXT("[%d]"), Index));
}

void FYamlParseResult::PopStack() {
    ScopesStack.RemoveAt(ScopesStack.Num() - 1);
}


const TArray<FString> UYamlReflection::NativeTypes = {"FString",    "FText",  "FVector",      "FVector2D", "FQuat",
                                                      "FTransform", "FColor", "FLinearColor", "FRotator"};


FYamlParseResult UYamlReflection::ParseNodeIntoObject_BP(const FYamlNode& Node, UObject* Object,
                                                         const FYamlParseIntoOptions& Options) {
    return {};
}


DEFINE_FUNCTION(UYamlReflection::execParseNodeIntoStruct_BP) {
    ...
    P_GET_STRUCT_REF(FYamlParseIntoOptions, Options);

    P_FINISH;

    if (!StructProperty || !StructValue) {
        const FString ErrorMessage = FString::Printf(
            TEXT("ParseNodeIntoStruct did not receive an Struct Property: '%s'"), *Stack.MostRecentProperty->GetName());
    ...
    ParseIntoStruct(Node, StructProperty->Struct, StructValue, Options, Result);
    *static_cast<FYamlParseResult*>(RESULT_PARAM) = Result;
...
void UYamlReflection::ParseIntoProperty(const FYamlNode& Node, const FProperty& Property, void* PropertyValue,
    ...
    if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(&Property)) {
        const int64 Index = ParseEnumValue(Node, EnumProperty->GetEnum(), Options.CheckEnums, Result);
        if (Index != INDEX_NONE) {
            EnumProperty->GetUnderlyingProperty()->SetIntPropertyValue(PropertyValue, Index);
        }
    } else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(&Property);
               ByteProperty && ByteProperty->IsEnum()) {
        const int64 Index = ParseEnumValue(Node, ByteProperty->GetIntPropertyEnum(), Options.CheckEnums, Result);
        if (Index != INDEX_NONE) {
        ...
        ParseScalarValue<bool>(Node, BoolProperty, PropertyValue, TEXT("Bool"), Options.StrictTypes, Result);
    } else if (const FStrProperty* StringProperty = CastField<FStrProperty>(&Property)) {
        ParseScalarValue<FString>(Node, StringProperty, PropertyValue, TEXT("String"), Options.StrictTypes, Result);
    } else if (const FNameProperty* NameProperty = CastField<FNameProperty>(&Property)) {
        ParseScalarValue<FName>(Node, NameProperty, PropertyValue, TEXT("String"), Options.StrictTypes, Result);
    } else if (const FTextProperty* TextProperty = CastField<FTextProperty>(&Property)) {
        ParseScalarValue<FText>(Node, TextProperty, PropertyValue, TEXT("String"), Options.StrictTypes, Result);
        ...
        for (int32 i = 0; i < Helper.Num(); ++i) {
            Result.PushStack(i);
            ParseIntoProperty(Node[i], *ArrayProperty->Inner, Helper.GetRawPtr(i), Options, Result);
            Result.PopStack();
        }
        ...
        for (const auto [Key, Value] : Node) {
            ...
            ParseIntoProperty(Key, *Helper.KeyProp, Helper.GetKeyPtr(i), Options, Result);
            ParseIntoProperty(Value, *Helper.ValueProp, Helper.GetValuePtr(i), Options, Result);
        ...
        return ParseIntoObject(Node, ObjectProperty->PropertyClass, PropertyValue, Options, Result);
    } else if (const FStructProperty* StructProperty = CastField<FStructProperty>(&Property)) {
        return ParseIntoStruct(Node, StructProperty->Struct, PropertyValue, Options, Result);
    }
}

void UYamlReflection::ParseIntoStruct(const FYamlNode& Node, const UScriptStruct* Struct, void* StructValue,
                                      const FYamlParseIntoOptions& Options, FYamlParseResult& Result) {
    UE_LOG(LogYamlParsing, Verbose, TEXT("ParseNodeIntoStruct: %s"), *Struct->GetName())

    // Custom type handlers provided in options have priority.
    if (const FCustomTypeHandler* Handler = Options.TypeHandlers.Find(Struct->GetStructCPPName())) {
    ...
    for (TFieldIterator<FProperty> It(Struct); It; ++It) {
        ...
        ParseIntoProperty(Node[Key], **It, It->ContainerPtrToValuePtr<void>(StructValue), Options, Result);
...
void UYamlReflection::ParseIntoObject(const FYamlNode& Node, const UClass* Object, void* ObjectValue,
                                      const FYamlParseIntoOptions& Options, FYamlParseResult& Result) {
    UE_LOG(LogYamlParsing, Verbose, TEXT("ParseNodeIntoObject: %s"), *Object->GetName())
    ...
    for (TFieldIterator<FProperty> It(Object); It; ++It) {
        ...
        Result.PushStack(Key);
        ParseIntoProperty(Node[Key], **It, It->ContainerPtrToValuePtr<void>(ObjectValue), Options, Result);
        Result.PopStack();
    }
}

void UYamlReflection::ParseIntoNativeType(const FYamlNode& Node, const UScriptStruct* Struct, void* StructValue) {
...
bool UYamlReflection::EnsureNodeType(const FYamlNode& Node, const EYamlNodeType Expected, const bool Strict,
                                     FYamlParseResult& Result) {
...
int64 UYamlReflection::ParseEnumValue(const FYamlNode& Node, const UEnum* Enum, const bool CheckEnums,
                                      FYamlParseResult& Result) {
    if (!Node.IsDefined()) {
        return INDEX_NONE;
    }
...
void UYamlReflection::ParseFromProperty(FYamlNode& Node, const FProperty& Property, const void* PropertyValue,
                                        const FYamlParseFromOptions& Options, FYamlParseResult& Result) {}

void UYamlReflection::ParseFromStruct(FYamlNode& Node, const UScriptStruct* Struct, const void* StructValue,
                                      const FYamlParseFromOptions& Options, FYamlParseResult& Result) {}

void UYamlReflection::ParseFromObject(FYamlNode& Node, const UClass* Object, const void* ObjectValue,
                                      const FYamlParseFromOptions& Options, FYamlParseResult& Result) {}

void UYamlReflection::ParseFromNativeType(FYamlNode& Node, const UScriptStruct* Struct, const void* StructValue,
                                          const FYamlParseFromOptions& Options, FYamlParseResult& Result) {}
